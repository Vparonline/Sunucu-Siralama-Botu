...
const discordsss = require("discord.js")
...
app.get('/like/list', async (req, res) => {
  ...
  client.guilds.cache.filter(async (guilds) => {
    ...
    await array.push({
      ...
      iconURL: guilds.iconURL({ dynamic: true }) ? guilds.iconURL({ dynamic: true }) : 'https://media.discordapp.net/attachments/1075400424871436328/1086631624441086074/sade.png?width=670&height=670',
      bannerURL: guilds.bannerURL() ? guilds.bannerURL() : 'https://cdn.discordapp.com/attachments/1075400424871436328/1086631624185225256/textdislist.png'
    })
  });
...
app.get('/server/:id', async (req, res) => {
  ...
  if (guild) {
    ...
    res.json({
      name: guild.name,
      id: guild.id,
      icon: guild.iconURL({ dynamic: true }) ? guild.iconURL({ dynamic: true }) : "https://cdn.discordapp.com/attachments/1075400424871436328/1086631624185225256/textdislist.png",
...
client.on("ready", async() => {
  let sunucuid = "1083856930323308774";
  let kanalid = "1083856930793078975";
  let mesajid = "1086779961102323713";
  
 let oyMsg = await client.guilds.cache.get(sunucuid).channels.cache.get(kanalid).messages.fetch(mesajid);

  let intervalCount = 5000 //1800000
  ...
  function gosterOycuklari() { 
  likeDatabase.find({ guildID: sunucuid }, async (err, res) => {
  res = res.filter(x => client.guilds.cache.get(sunucuid).members.cache.get(x.userID));
  const oyList = res.filter(x => x && x.likeCount !== 0).sort((x, y) => y.likeCount - x.likeCount).map((val, i) =>`\`${i + 1}.\` ${client.guilds.cache.get(sunucuid).members.cache.get(val.userID)}: ${val.likeCount} Oy`).splice(0, 30).join("\n");
  let chat = new EmbedBuilder()
  chat.setDescription(`Oy Sıralaması | Tüm Zamanlar\n\n${oyList.length === 0 ? `Veritabanında kayıtlı mesaj verisi yok bu yüzden bilgileri gösteremiyorum.` : oyList}` )
  oyMsg.edit({ embeds: [chat]})})}
...
client.on("voiceStateUpdate", async (oldV, newV) => {
  ...
  client.guilds.cache.forEach(async (guilds) => {
    ...
    array.push({
      guildName: guilds.name,
      guildID: guilds.id,
      guildicon: guilds.iconURL({ dynamic: true }) ? guilds.iconURL({ dynamic: true }) : "https://cdn.discordapp.com/attachments/1075400424871436328/1086631624185225256/textdislist.png",
      count: size,
      categoryName: name
    })
  })
